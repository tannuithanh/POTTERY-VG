            ...
            <a-col :span="12">
                <a-form-item label="Tệp đính kèm" name="attachment">
                    ...
                    <div v-if="existingFileUrl && !fileList.length" class="mt-1 text-xs">
                        File hiện có:
                        <a :href="existingFileUrl" target="_blank" rel="noopener">{{ existingFileName }}</a>
                        <a class="ml-2" @click="markRemoveExisting">Xóa</a>
                    </div>
                </a-form-item>
            </a-col>
...
const fileList = ref([]);                 // file mới (1 file)
const existingFileUrl = ref(null);        // url file hiện có
const existingFileName = ref('file');     // tên file hiện có
const removeExisting = ref(false);        // cờ xóa file cũ khi upload mới
...
const rules = {
    ...
    related_people: [{
        validator: (_, v) => (v && v.length > 0) ? Promise.resolve() : Promise.reject('Phải chọn ít nhất một người liên quan'),
    }],
};

onMounted(async () => {
    if (!id) {
        notification.error({ message: 'Lỗi', description: 'Không tìm thấy ID cuộc họp' });
        router.back(); return;
    }
...
async function loadDetail() {
    try {
        const r = await meetingRoomService.get(id, { include_users: true });
        const m = r?.data?.data ?? r?.data ?? r;

        const toNum = (v) => (v === null || v === undefined || v === '' ? null : Number(v));
        const toNumArr = (arr) => (Array.isArray(arr) ? arr.map((x) => Number(x)) : []);

        Object.assign(form.value, {
            room: toNum(m.room_id ?? m.room?.id),
            title: m.title ?? '',
            start_datetime: dayjs(m.start_at),
            end_datetime: dayjs(m.end_at),
            related_people: toNumArr(m.stakeholder_user_ids ?? []),
            devices: Array.isArray(m.room_devices) ? m.room_devices.join(', ') : '',
            specialists: toNumArr(m.specialist_user_ids ?? []),
            advisors: toNumArr(m.advisor_user_ids ?? []),
            decision_maker: toNum(m.decision_maker_user_id),
            secretary_user_id: toNum(m.secretary_user_id),
            moderator: m.chairperson_name ?? '',
            note: m.notes ?? '',
            result_record_location: m.result_location ?? '',
        });

        existingFileUrl.value = m.file_url ?? m.file?.url ?? m.files?.[0]?.url ?? null;
        existingFileName.value = m.file_name ?? 'file';
    } catch (e) {
        notification.error({ message: 'Lỗi', description: 'Không tải được dữ liệu cuộc họp' });
    }
}

const beforeUpload = (file) => {
    const ok = file.size / 1024 / 1024 < 8;
    if (!ok) { notification.error({ message: 'Tệp quá lớn', description: 'Dung lượng > 8MB' }); return false; }
    fileList.value = [file];
    removeExisting.value = true;
    return false;
};
const onRemoveFile = () => { fileList.value = []; };

function validateDateTime() {
    const a = dayjs(form.value.start_datetime), b = dayjs(form.value.end_datetime);
    if (a.format('YYYY-MM-DD') !== b.format('YYYY-MM-DD')) throw new Error('Ngày bắt đầu và kết thúc phải giống nhau');
    if (a.isAfter(b)) throw new Error('Giờ bắt đầu phải nhỏ hơn giờ kết thúc');
}
function validateRoles() {
    ...
    const s = new Set(all);
    if (s.size !== all.length) throw new Error('Một người không được chọn trùng vai trò');
}
...
function buildPayload() {
    ...
    return {
        ...
        decision_maker_user_id: form.value.decision_maker ?? null,
        secretary_user_id: form.value.secretary_user_id ?? null,
        chairperson_name: form.value.moderator || '',
        notes: form.value.note || '',
        result_location: form.value.result_record_location || '',
        remove_existing_files: removeExisting.value ? 1 : 0, // BE tuỳ chọn xử lý
    };
}
...
async function onSubmit() {
    try {
        ...
        try { validateDateTime(); validateRoles(); }
        catch (msg) { notification.error({ message: 'Lỗi', description: String(msg) }); return; }

        const payload = buildPayload();

        // UPDATE
        await meetingRoomService.update(id, payload);

        // Upload file mới (nếu có)
        if (fileList.value.length) {
            await meetingRoomService.uploadFile(id, fileList.value[0]);
        }

        notification.success({ message: 'Thành công', description: 'Cập nhật lịch họp thành công!' });
        router.push({ name: 'meetingManager' });
    } catch (e) {
        handleSubmitError(e);
    } finally {

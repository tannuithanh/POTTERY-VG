<template>
    <teleport to="body">
        <div class="rf-overlay" :style="{ zIndex: zIndex.toString() }" aria-hidden="true">
            <div class="rf-layer">
                <span v-for="p in petals" :key="p.id" class="rf-petal" :style="styleFor(p)"
                    @mouseenter="nudge(p)">🌹</span>
            </div>
        </div>
    </teleport>
</template>
...
function nudge(p) { p.push = (Math.random() < .5 ? -1 : 1) * (12 + Math.random() * 18); p.rot += 360; setTimeout(() => p.push = 0, 400) }
...
<style scoped>
/* QUAN TRỌNG: overlay cho phép pointer events để petal nhận hover */
.rf-overlay {
    position: fixed;
    inset: 0;
    pointer-events: auto;
